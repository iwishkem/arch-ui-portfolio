...
import MobileUI from './components/MobileUI'; // Import the new mobile UI
import { Terminal, User, Folder as FolderIcon, RefreshCw, Monitor, Settings } from 'lucide-react';
...
export default function App() {
  ...
  const [volume, setVolume] = useState<number>(() => {
    const savedVolume = localStorage.getItem('archVolume');
    return savedVolume ? JSON.parse(savedVolume) : 66;
  });

  const displayBg = customBg || activeTheme.bg;

  const defaultPositions: Record<string, {x: number, y: number}> = { 
    terminal: { x: 20, y: 20 }, about: { x: 20, y: 124 }, files: { x: 20, y: 228 }, settings: { x: 20, y: 332 }
  };
  ...
  const [resets, setResets] = useState<Record<string, number>>({ terminal: 0, files: 0, about: 0, settings: 0 });
  ...
  const handleSetVolume = (newVolume: number) => {
    setVolume(newVolume);
    localStorage.setItem('archVolume', JSON.stringify(newVolume));
  };
  ...
  if (isMobile) {
    return <MobileUI 
      ...
      TweakToolContent={TweakToolContent}
      AboutMeContent={<div className="p-6 text-gray-300 font-mono text-xs leading-relaxed overflow-auto h-full">
      <p className="text-[#a6e3a1] mb-2 font-bold underline"># Profile: Kem</p><p>- Student in Turkey</p><p>- Android Modding (S21FE / LineageOS)</p><p>- Developer: Flutter, Python, Reverse Engineering</p></div>}
    />;
  }
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col transition-all duration-700 font-sans" style={{ backgroundImage: `url("${displayBg}")`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <TopPanel activeWorkspace={activeWorkspace} setActiveWorkspace={setActiveWorkspace} theme={activeTheme} openTerminal={() => openWindow('term', 'kem@arch: ~', <TerminalApp onExit={() => closeWindow('term')} />)} openSingleWindow={openWindow} volume={volume} setVolume={handleSetVolume} />
